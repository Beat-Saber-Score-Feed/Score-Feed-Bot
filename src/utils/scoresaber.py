...
pp_multiplier = 42.117208413

def calculate_pp(acc, stars):
    return round(stars * pp_multiplier * interpolation.interpolate(acc, curve), 2)

def calculate_acc(target_pp, stars):
    tolerance = 0.0001

    max_possible = stars * (pp_multiplier * interpolation.interpolate(100, curve))
    ...
    while high - low > tolerance:
        mid = (low + high) / 2
        pp = stars * (pp_multiplier * interpolation.interpolate(mid, curve))
